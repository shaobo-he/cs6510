...
(define (expr-i->c [a : ExprI] [super-name : symbol]) : ExprC
  (local [(define (recur expr)
    ...
    (type-case ExprI a
      ...
      [instanceofI (obj-expr name)
                   (instanceofC (recur obj-expr) name)])))
  ...
  (type-case ClassC c
    [classC (name super field-names methods)
            (type-case ClassC (flatten-super name classes i-classes)
              [classC (super-name super super-field-names super-methods)
                      (classC
                       name
                       super-name
...
(define (interp-prog [classes : (listof s-expression)] [a : s-expression]) : s-expression
  (let ([v (interp-i (parse a)
                     (map parse-class (cons object-class classes)))])
    (type-case Value v
